...
/// This converts between the OpenAI Realtime SDK and Microsoft.Extensions.AI.Abstractions primitives.
/// </summary>
public static class OpenAIRealtimeExtensions
{
    ...
    {
        ThrowIfNull(aiFunction);

        BinaryData functionParameters = new(JsonSerializer.SerializeToUtf8Bytes(aiFunction.JsonSchema));
    ...
    private static void ThrowIfNull<T>(T value, [CallerArgumentExpression(nameof(value))] string? name = null) where T : class
    {
        if (value is null)
        {
